...
class FastLEDMatrixOutput : public light::AddressableLight {
 public:
  ...
  void show();
  void set_matrix(uint8_t width, uint8_t height, uint8_t matrixType);
  void set_max_brightness(uint8_t maxBrightness);
  void set_max_current(uint32_t maxCurrent);
  void set_rotation(uint8_t rotation);

  MyMatrix *matrix_{nullptr};
